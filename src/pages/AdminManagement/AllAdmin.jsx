...
const AllAdmin = () => {
  ...
  // FIX: Removed setError(null) from resetForm so error persists after close
  const resetForm = () => {
    setFormData({
      name: "",
      mobile: "",
  ...
  // FIX: Clear error only when opening modal fresh
  ...
  const handleCloseModal = () => {
    setIsModalOpen(false);
    resetForm();
    setError(null); // Clear error when user explicitly closes modal
  };
  ...
  return (
    <DashboardLayout>
      ...
      {/* Endpoint Not Found Warning — only when modal is closed */}
      ...
      {/* FIX: Outer error banner only shows when modal is NOT open */}
      {error && !isModalOpen && (
        <div className="mb-4 mx-4 bg-red-50 border-l-4 border-red-500 text-red-700 px-6 py-4 rounded-xl flex items-center gap-3 shadow-md">
          <X size={24} />
          <span className="font-medium">{error}</span>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 pl-4 max-w-[99%] mx-auto mt-2 mb-2">
        ...
        {/* FIX: Add Admin Button now calls handleOpenModal */}
        <div className="w-full md:w-auto flex justify-start md:justify-end mt-2 md:mt-0">
          <button
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            ...
            {error && (
              <div className="mx-4 mt-4 bg-red-50 border-l-4 border-red-500 text-red-700 px-6 py-4 rounded-xl flex items-center gap-3 shadow-md">
