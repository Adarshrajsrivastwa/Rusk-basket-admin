...
//             line1:
//               addressParts.line1 || results[0].formatted_address || prev.line1,
//           }));
//         }
//       });
//     });
...
//                   Manage and monitor delivery executive job postings across all
//                   vendors
//                 </p>
//               </div>
//               <button
...
//                                 () => {
//                                   showNotification(
//                                     "Unable to fetch location. Please use map to select.",
//                                     "error",
//                                   );
//                                 },
//                               );
//                             } else {
//                               showNotification(
//                                 "Geolocation not supported. Please use map.",
//                                 "error",
//                               );
//                             }
//                           }}
...
//                           Click on map to select location or search for an
//                           address
//                         </p>
//                       </div>
//                       <button
...
//                                   Please wait while we load OpenStreetMap
//                                 </p>
//                               </div>
//                             </div>
//                           ) : (
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        showNotification(data.message || "Failed to fetch jobs", "error");
      }
    } catch {
      showNotification("Failed to fetch jobs", "error");
    } finally {
  ...
  const JobFormModal = () => (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50">
      <div className="bg-white rounded-sm shadow-2xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        {/* Modal Header */}
        <div className="bg-[#FF7B1D] px-6 py-4 flex justify-between items-center">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          {/* Job Details */}
          <div>
            <h3 className="text-sm font-bold text-gray-700 uppercase tracking-wide mb-3 border-b border-gray-200 pb-2">
              Job Details
            </h3>
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-[#FF7B1D] bg-white"
                >
                  <option value="">Select a vendor</option>
                  {vendors.map((vendor) => (
                    <option key={vendor._id} value={vendor._id}>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-[#FF7B1D]"
                      placeholder="23.2599"
                    />
                    <button
                      type="button"
                      ...
                      className="px-3 py-2 bg-[#FF7B1D] hover:bg-orange-600 text-white rounded text-sm"
                      title="Get Current Location"
                    >
                      📍
                    </button>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button
                    type="button"
                    onClick={() => setShowMap(!showMap)}
                    className="flex items-center gap-1 px-3 py-1.5 bg-[#FF7B1D] hover:bg-orange-600 text-white rounded text-xs font-semibold"
                  >
                    <MapPin size={14} />
                    {showMap ? "Hide Map" : "Show Map"}
                  </button>
                ...
                {showMap && (
                  <div className="border border-gray-300 rounded overflow-hidden">
                    <div className="bg-[#FF7B1D] px-4 py-2 flex items-center justify-between">
                    ...
                    <div className="bg-gray-50 px-3 py-2 border-b border-gray-200 flex gap-2">
                      <div className="flex-1 relative">
                        ...
                        <input
                          ...
                          className="w-full pl-8 pr-3 py-1.5 border border-gray-300 rounded text-sm focus:outline-none focus:border-[#FF7B1D]"
                          onKeyPress={async (e) => {
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-[#FF7B1D] hover:bg-orange-600 text-white rounded text-xs font-semibold whitespace-nowrap"
                      >
                        📍 Current
                      </button>
                    </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
            <button
              ...
              className="px-5 py-2 border border-gray-300 rounded hover:bg-gray-50 text-sm font-semibold"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="px-6 py-2 bg-black hover:bg-orange-600 text-white rounded text-sm font-semibold flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              {loading ? (
                <>
  ...
  return (
    <DashboardLayout>
      <div className="min-h-screen p-0 ml-6">
        ...
        {notification.show && (
          <div
            className={`fixed top-6 right-6 z-50 flex items-center gap-3 px-5 py-3 rounded shadow-lg text-white text-sm font-medium ${notification.type === "success" ? "bg-green-500" : "bg-red-500"}`}
          >
            {notification.type === "success" ? (
              <Check size={18} />
            ) : (
        ...
        {/* Top Bar — matches CreateCategory exactly */}
        <div className="flex flex-col md:flex-row md:items-center pl-4 md:justify-between gap-3 max-w-[99%] mx-auto mt-2 mb-2">
          ...
          <div className="w-full md:w-auto flex justify-start md:justify-end mt-2 md:mt-0">
            <button
              ...
              className="bg-black text-white w-52 sm:w-60 px-4 sm:px-5 py-2 rounded-sm shadow hover:bg-orange-600 text-xs sm:text-sm flex items-center justify-center gap-2 whitespace-nowrap transition-colors"
            >
              <Plus size={16} />
              Create Job Post
            </button>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#FF7B1D] text-black">
                ...
                <th className="p-3 text-center">Joining Bonus</th>
                <th className="p-3 text-center">Onboarding Fee</th>
              ...
              <tbody>
                {currentJobs.map((job, idx) => (
                  ...
                  >
                    ...
                    <td className="p-3 text-center">
                      <span
                        className={`font-semibold ${job.isActive ? "text-green-600" : "text-gray-500"}`}
                      >
                        {job.isActive ? "Active" : "Inactive"}
                      </span>
                    </td>
              ...
              <div className="p-6 space-y-3 text-sm">
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <p
                      className={`font-semibold ${selectedJob.isActive ? "text-green-600" : "text-gray-500"}`}
                    >
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-green-600">
                      ₹{selectedJob.joiningBonus?.toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <p className="text-xs text-gray-500 font-medium">
                    ...
                    <p className="font-semibold text-blue-600">
                      ₹{selectedJob.onboardingFee?.toLocaleString()}
                    </p>
                  </div>
                  ...
                  <div className="col-span-2 border-t pt-3">
                    ...
                    {selectedJob.location?.latitude && (
                      <p className="text-gray-500 text-xs mt-1">
                        Coords: {selectedJob.location.latitude},{" "}
                ...
                <div className="flex justify-end pt-2">
                  <button
                    ...
                    className="px-5 py-2 bg-black hover:bg-orange-600 text-white rounded text-sm font-semibold transition-colors"
                  >
                    Close
                  </button>
                </div>
              ...
              <div className="p-6 text-center">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    onClick={() => deleteJobPost(selectedJob._id)}
                    disabled={loading}
                    className="flex-1 px-4 py-2 bg-red-500 hover:bg-red-600 text-white rounded text-sm font-semibold flex items-center justify-center gap-2 disabled:opacity-50 transition-colors"
                  >
                    {loading ? (
                      <>
                        <Loader2 className="animate-spin" size={16} />
